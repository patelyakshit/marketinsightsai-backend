        ...
        try:
            ...
            if not image_saved:
                ...
                try:
                    ...
                    print(f"Imagen 4 fallback failed: {imagen_error}")
                    image_url = "https://placehold.co/1080x1080/3b82f6/ffffff?text=Image+Generation+Unavailable"

        except Exception as e:
            print(f"Marketing image generation error: {e}")
...
- NEVER mention lack of data or missing segments - work with what is provided"""
    ...
    system_prompt = f"You are a retail location analytics expert specializing in {goal or 'marketing'} strategy. The segments provided ARE the top 5 customer segments in the trade area - always reference them by name and provide specific recommendations for serving these customers. Write approximately 120 words."
...
Write EXACTLY 50 words of specific, actionable advice for reaching and engaging this segment. Be direct and practical. Include one specific tactic or channel recommendation."""
    ...
    response = await client.chat.completions.create(
        model=settings.openai_model,
        messages=[
            {"role": "system", "content": "You are a concise retail marketing expert. Write exactly 50 words, no more, no less."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=100,
        temperature=0.7,
